...
- **Transactional deletes** — document removal cascades through ChromaDB → FTS5 → SQLite atomically; any failure at a step returns a 500 before the next layer is touched
...
The file is saved with a `{uuid4_hex}_{secure_filename}` name to prevent path traversal, then an `indexing_task` closure is enqueued. The endpoint returns `202 Accepted` immediately — the caller is never blocked by embedding time.
...
If you found LectureLens interesting, here are the other projects on my portfolio:
